...
func Bypass(urls <-chan string, output chan<- string, wg *sync.WaitGroup) {
	...
	for url := range urls {
		...
		if resp.StatusCode == 403 {
			bypassheaders = append(bypassheaders, headers{name: "Referer", value: url})
			if parsedURL.Path != "" {
				bypassheaders = append(bypassheaders, headers{name: "X-Origial-URL", value: parsedURL.Path})
			}
			for _, header := range bypassheaders {
				...
				CheckError(err)
				if resp.StatusCode != 403 {
					output <- fmt.Sprintf("Check Manually! Url: %v\nUsing: %v: %v (%v)", url, header.name, header.value, resp.Status)
				}
			}
		}
	}
